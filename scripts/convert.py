...
def doc_rename(path_src):
    index = 0
    imags = os.listdir(path_src)
    imags=sorted(imags)
    for i in imags:
        print(i.encode('utf-8', 'replace').decode())
        doc_src = os.path.join(path_src, i)
        if index < 10:
            name = '000' + str(index) + '.jpg'
        elif index < 100:
            name = '00' + str(index) + '.jpg'
        elif index < 1000:
            name = '0' + str(index) + '.jpg'
        else:
            name = str(index) + '.jpg'
        # 确定目标路径
        doc_dst = os.path.join(path_src, name)
